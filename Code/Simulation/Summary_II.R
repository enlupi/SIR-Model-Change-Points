...
for(a in Xuan)
{
  Result<-rbind(Result,read.csv(paste(c("Day100II_",a,".csv"),collapse = ""),header = T)[,-1])
}
...
png("Day100II_Summary.png",400,500)
...
png("Day100II_delta_Summary.png",600,400)
par(mar=c(5,5,2,2),mfrow=c(1,1))
plot(2:100,(colMeans(Result[1+0:(Ya*10-1)*6,-1])),xlim=c(0,100),ylim = c(0,0.4),main = "",ylab="",xlab = "",type="b",pch=18,lwd=2,col=2, xaxs="i", yaxs="i",xaxt="n", yaxt="n",cex=2)
mtext("Day",side=1,line = 3,cex=2)
...
axis(2,at=0:4/10,labels = as.character(0:4/10),cex.axis=2)
for(i in 1+0:3*25)
{
  abline(v=i,col=1,lty=2)
}
...
png("Day100II_CP_Summary.png",600,400)
par(mar=c(5,5,2,2),mfrow=c(1,1))
